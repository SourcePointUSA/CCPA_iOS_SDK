...
class ConsentViewDetailsViewController: BaseViewController, WKNavigationDelegate, ConsentDelegate {
    ...
    func setconsentIdFromUserDefaults() {
        if let consentID = UserDefaults.standard.string(forKey: CCPAConsentViewController.CONSENT_UUID_KEY),consentID.count > 0 {
            ...
            euConsentLabel.text = UserDefaults.standard.string(forKey: CCPAConsentViewController.EU_CONSENT_KEY)
        }else {
    ...
    func loadConsentManager(propertyDetails : PropertyDetailsModel, targetingParamsArray:[TargetingParamModel]) {
        do {
            let consentViewController = try CCPAConsentViewController (accountId: Int(propertyDetails.accountId), propertyId: Int(propertyDetails.propertyId), property: propertyDetails.property!, PMId: propertyDetails.privacyManagerId!, campaign: propertyDetails.campaign, showPM: propertyDetails.showPM, consentDelegate: self)
